...
#     Reconstructs the hierarchical child steps as a clean array.
#     """
#     if not ls_client:
#         # raise HTTPException(
#         #     status_code=503, 
...
#     except Exception as e:
#         # Catch the 404 if LangSmith hasn't ingested the trace yet
...
#         child_runs = list(ls_client.list_runs(trace_id=run_id))
#     except Exception as e:
#         child_runs = [] # Default to empty if the child query fails
...
#             "latency_ms": (child.end_time - child.start_time).total_seconds() * 1000 if child.end_time and child.start_time else 0,
...
async def get_agent_trace(run_id: str):
    ...
    for attempt in range(25):  # Up to ~50 seconds
        try:
            run = ls_client.read_run(run_id)
